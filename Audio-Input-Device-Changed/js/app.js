...
(function closure() {
  ...
  const currentAudioInput = document.querySelector('#current-audio-input');
  let publisher;
  ...
  publishBtn.addEventListener('click', () => {
    ...
    publisher = OT.initPublisher('publisher', {
      ...
      if (err) {
        ...
        currentAudioInput.innerText = audioSelector.options[audioSelector.selectedIndex].innerText;
    ...
    publisher.on('audioInputDeviceChanged', (event) => {
      currentAudioInput.innerText = event.device.label;
    });
  ...
  function setupDeviceSwitching() {
    ...
    audioSelector.addEventListener('change', (event) => {
      ...
      publisher.setAudioSource(event.target.value).then(() => {
        audioSelector.disabled = false;
        currentAudioInput.innerText = event.target.options[event.target.selectedIndex].innerText;
      }).catch((err) => {
